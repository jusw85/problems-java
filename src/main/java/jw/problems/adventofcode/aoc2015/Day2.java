...
public class Day2 {
    ...
    public static Pattern p = Pattern.compile("(\\d+)x(\\d+)x(\\d+)");
    ...
    public static class Box {
        ...
        public int surfaceArea() {
            ...
            int s2 = w * h;
            int s3 = h * l;
            int smallestSide = Math.min(Math.min(s1, s2), s3);
            return 2 * (s1 + s2 + s3) + smallestSide;
        }

        public int ribbonLength() {
            int longestSide = Math.max(Math.max(l, w), h);
            int perimeter = 2 * (w + l + h - longestSide);
            int vol = l * w * h;
            return perimeter + vol;
        }
    }

    public static void main(String[] args) throws FileNotFoundException {
        Scanner sc = new Scanner(new File("./etc/aoc2015/in2"));
        ...
        while (sc.hasNextLine()) {
            ...
            Box b = new Box(l, w, h);
            totalSurfaceArea += b.surfaceArea();
