...
public class SkynetRevolution1 {
    ...
    public static void main(String args[]) {
        ...
        while (true) {
            ...
            Queue<LinkedList<Integer>> toVisit = new LinkedList<>();
            ...
            while (!toVisit.isEmpty()) {
                LinkedList<Integer> currpath = toVisit.poll();
                int currentNode = currpath.getLast();
                for (int i = 0; i < N; i++) {
            ...
            if (paths.size() > 0) {
                LinkedList<Integer> minpath = paths.poll();
                int minsize = minpath.size();
                for (LinkedList<Integer> currpath : paths) {
                    if (currpath.size() < minsize) {
                        minsize = currpath.size();
                        minpath = currpath;
                    }
                }
                sever(graph, minpath.get(0), minpath.get(1));
            }

        }
    }

    public static void sever(int[][] graph, int n1, int n2) {
